...
public class AuthController : ControllerBase
{
    ...
    public async Task<IActionResult> Register([FromBody] RegisterDto dto)
    {
        ...
        user.PasswordExpiryDate = DateTime.UtcNow.AddDays(_config.GetValue<int>("Security:PasswordExpiryDays", 90));
    ...
    { 
        ...
        var user = await _users.GetByIdAsync(userId); if (user == null) return NotFound(); user.EmailVerified = true; await _users.UpdateAsync(user); await _users.SaveChangesAsync(); return Ok(new { message = "Email verified" }); }
    [HttpPost("verify-mobile")] public async Task<IActionResult> VerifyMobile([FromBody] dynamic body) { int userId = (int)body.userId; var user = await _users.GetByIdAsync(userId); if (user == null) return NotFound(); user.MobileVerified = true; await _users.UpdateAsync(user); await _users.SaveChangesAsync(); return Ok(new { message = "Mobile verified" }); }
    ...
    public async Task<IActionResult> Login([FromBody] LoginDto dto)
    {
        ...
        if (!VerifyHashedPassword(user.PasswordHash, dto.Password))
        {
            ...
            if (user.FailedLoginCount >= _config.GetValue<int>("Security:Lockout:MaxFailedAttempts", 5))
            {
                user.LockoutEndAt = DateTime.UtcNow.AddMinutes(_config.GetValue<int>("Security:Lockout:LockoutMinutes", 15));
    ...
    [HttpPost("refresh")] public async Task<IActionResult> Refresh([FromBody] RefreshDto dto) { var rt = await _rtService.GetValidRefreshTokenAsync(dto.RefreshToken); if (rt == null) return Unauthorized(); var newRt = await _rtService.CreateRefreshTokenAsync(rt.UserId, HttpContext.Connection.RemoteIpAddress?.ToString() ?? "unknown", 30); await _rtService.RevokeAsync(rt, "rotated", newRt.Token); var user = await _users.GetByIdAsync(rt.UserId); if (user == null) return Unauthorized(); var accessToken = GenerateJwt(user); return Ok(new { accessToken, refreshToken = newRt.Token }); }
    [HttpPost("logout")] public async Task<IActionResult> Logout([FromBody] dynamic body) { string token = (string)body.refreshToken; var rt = await _rtService.GetValidRefreshTokenAsync(token); if (rt != null) await _rtService.RevokeAsync(rt, "logout"); return Ok(new { message = "Logged out" }); }
    ...
    private string GenerateJwt(User user)
    {
        ...
        var claims = new[] { new Claim(JwtRegisteredClaimNames.Sub, user.Id.ToString()), new Claim(ClaimTypes.Email, user.Email), new Claim(ClaimTypes.Name, user.FullName), new Claim(ClaimTypes.Role, user.Role?.Name ?? "Customer") };
        var token = new JwtSecurityToken(issuer: _config["Jwt:Issuer"], audience: _config["Jwt:Audience"], claims: claims, expires: DateTime.UtcNow.AddMinutes(_config.GetValue<int>("Jwt:ExpiryMinutes", 15)), signingCredentials: creds);
